...
def create_level(SIZE:tuple[int]):
    ...
    mainCharacter = MainCharacter(Physics([50,200], isCollidable=True, hitboxSize=[50,50], hitboxLocation=Location.CENTER), Animation(textures=mcTextures, actualTexture='Idle', frameLimit=0), location=Location.CENTER)
    ...
    for i in range(10):
        ...
        for j in [100, 175, 350, 425]:
            chairList.append(Entity(Physics([100 + 100*i, j], margin=[0, -50, 0, 0], isCollidable=True), Animation(textures=ch1Textures, actualTexture='chair1'), collideListeners={"MainCharacter": chair_negative}))
